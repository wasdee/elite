...
def test_command_creates_inexistent(tmpdir):
    p = tmpdir.join('test')
...
def test_command_creates_exists(tmpdir):
...
def test_command_removes_inexistent(tmpdir):
    p = tmpdir.join('test')
...
def test_command_removes_exists(tmpdir):
    p = tmpdir.join('test').ensure()
...
def test_command_unless_ok():
    run = Run(command=['echo', '-n', 'hi'], unless='true')
    assert run.process() == ActionResponse(changed=False)


def test_command_unless_failed():
